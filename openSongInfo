...
<span class="genre-tag ${genreClass(song.genre)}">
${song.genre || ""}
</span>
...
`<span class="genre-tag ${genreClass(song.genre)}">${song.genre || ""}</span>`;

const sourceNames = {
guitarhero:"Guitar Hero",
guitarhero2:"Guitar Hero II",
guitarherowarriorsofrock:"Guitar Hero: Warriors of Rock",
guitarherowarriorsofrockdlc:"Guitar Hero: Warriors of Rock DLC",
rockbanddlc:"Rock Band 1 DLC",
fortnitefestival:"Fortnite Festival"
};

const sourceName = sourceNames[song.category] || "";

document.getElementById("info-source").innerHTML =
`${sourceName} ${song.category ? `<img class="source-icon" src="./assets/${song.category}.png">` : ""}`;
...
`<span class="song-rating ${rating}">${ratingText}</span>`;

let charter = song.charter || "";

if(["guitarhero","guitarhero2","rockbanddlc","fortnitefestival"].includes(song.category)){
